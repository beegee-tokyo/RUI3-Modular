...
bool RAK_PMSA003I::begin(TwoWire &wirePort, uint8_t deviceAddress)
{
  ...
  LIB_LOG("ID = 0x%X", sensor_id);
  delay(100);

  if(sensor_id == PMSA003I_CHIP_ID) 
  {
    LIB_LOG("PMSA003I ID read success.");
    return true;
  }
  else   
  {    
    LIB_LOG("PMSA003I ID read fail.");                             
    return false;
  }
}
...
bool RAK_PMSA003I::readDate(PMSA_Data_t *pmsa_data)
{
  ...
  for (uint8_t i = 0; i < 16; i++) 
  {
    buf16[i] = buf8[i*2 + 1];
    buf16[i] += (buf8[i*2] << 8);
    LIB_LOG("buf16[%d] = %x",i,buf16[i]);
  }

  memcpy((void *)pmsa_data, (void *)buf16, sizeof(PMSA_Data_t));
  LIB_LOG("pmsa_data.header = %x",pmsa_data->header);
  LIB_LOG("pmsa_data.date_len = %x",pmsa_data->date_len);

  LIB_LOG("sum = %x",sum);
  LIB_LOG("pmsa_data->check_sum = %x",pmsa_data->check_sum);
  
  if (sum != pmsa_data->check_sum) 
  {
    LIB_LOG("PMSA003I read date fail.");
    return false;
  }
  else
  {
    LIB_LOG("PMSA003I read date success.");
    return true;
  }
  return true;
}
...
//  else if(error == 1)
//  {
//    LIB_LOG("Data too long");
//  }
//  else if(error == 2)
//  {
//    LIB_LOG("NACK on transmit of address");
//  }
//  else if(error == 3)
//  {
//    LIB_LOG("NACK on transmit of data");
//  }
//  else if(error == 4)
//  {
//    LIB_LOG("Other error");
